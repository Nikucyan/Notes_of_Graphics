...
public class implicit_model : MonoBehaviour
{
	...
	float 		rho		= 0.995f;
	float 		spring_k = 8000;
	int[] 		E;	// vetex index (paired with edges)
	...
	{	// the gradient of the objective function
		...
		//Spring Force.
		for (int e = 0; e < E.Length/2; e++) {
			int v_i = E[e*2];	// set the two vertices
			int v_j = E[e*2+1];
			Vector3 L_curr = X[v_i] - X[v_j];
			G[v_i] += spring_k * (1 - L[e] / (L_curr.magnitude)) * (L_curr);	// i
			G[v_j] -= spring_k * (1 - L[e] / (L_curr.magnitude)) * (L_curr);	// j
		}	
	}
	...
	void Update () 
	{
		...
		Vector3[] G 		= new Vector3[X.Length];	// Gradient g
		...
		for(int k=0; k<32; k++)
		{
			...
			// Finishing pt.1 Updating position
			for (int i = 1; i < X.Length; i++) {
				if (i != 20) {
					Vector3[] old_X = new Vector3[X.Length];
					old_X[i] = X[i];
					X[i] += -1 / (mass / (t * t) + 4 * spring_k) * G[i];	// normal update
		...
		//Finishing. pt.2 updating velocity
		for (int i = 1; i < X.Length; i++) {
			if (i != 20) {
				V[i] += (X[i] - X_hat[i]) / t;
			}
