...
canvas = ti.Vector.field(3, dtype=ti.f32, shape=(image_width, image_height))    # 3 for RGB (3-channels)
...
@ti.kernel
def render():
    for i, j in canvas:     # outermost (parallelized)
        u = (i + ti.random()) / image_width     # random -> anti-aliasing
        v = (j + ti.random()) / image_height
        color = ti.Vector([0.0, 0.0, 0.0])
        for n in range(samples_per_pixel):
            ray = camera.get_ray(u, v)  # emit ray at u, v 
            color += ray_color(ray)     # shade -> color (summation)
        color /= samples_per_pixel      # every frame 4 times -> divide 
        canvas[i, j] += color
            ...
            if material == 0:
                ...
                if material == 1:
                    ...
                    if sample_on_unit_sphere_surface:
                        ...
                        target += random_in_unit_sphere()   # random sampling in space
                    scattered_direction = target - hit_point    # select a random dir
...
if __name__ == "__main__":  # Main loop
    parser = argparse.ArgumentParser(description='Naive Ray Tracing')  # adjust in terminal
    parser.add_argument(
    ...
    scene = Hittable_list()     # All stuffs in the scene (At first empty scene)

    # Light source
    scene.add(Sphere(center=ti.Vector([0, 5.4, -1]), radius=3.0, material=0, color=ti.Vector([10.0, 10.0, 10.0])))  # Actually color=1 (luminance higher)
    # Ground
    scene.add(Sphere(center=ti.Vector([0, -100.5, -1]), radius=100.0, material=2, color=ti.Vector([0.8, 0.8, 0.8])))    
    # ceiling
    scene.add(Sphere(center=ti.Vector([0, 102.5, -1]), radius=100.0, material=1, color=ti.Vector([30/256, 63/256, 75/256])))
    # back wall
    scene.add(Sphere(center=ti.Vector([0, 1, 101]), radius=100.0, material=1, color=ti.Vector([0.8, 0.8, 0.8])))
    # right wall
    scene.add(Sphere(center=ti.Vector([-101.5, 0, -1]), radius=100.0, material=1, color=ti.Vector([178/256, 151/256, 122/256])))
    # left wall
    scene.add(Sphere(center=ti.Vector([101.5, 0, -1]), radius=100.0, material=1, color=ti.Vector([0.7, 0.5, 0.6])))

     # materials: def as 0 - lightsource; 1 - diffuse; 2 - smooth metal; 3 - glass; 4 - rough-surface metal
    # Diffuse ball
